...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
        if (err_status == ESP_OK) {
            ...
            if (esp_zb_bdb_is_factory_new() && !commissioning_in_progress) {
                ...
                ESP_LOGI(TAG, "Device already commissioned or commissioning in progress");
            }
        } else {
            ESP_LOGE(TAG, "Startup signal error: %s", esp_err_to_name(err_status));
        }
        break;
